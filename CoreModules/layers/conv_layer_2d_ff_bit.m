function [ y, dzdw,dzdb,opts] = conv_layer_2d_ff_bit( I,kernel,bias,stride,pad,dzdy,opts )
...
%     W=abs(conv2d_nntb.Bias.Value);
%     nn=opts.parameters.batch_size;
%     Alpha=W/nn;
    if isempty(dzdy)
        y=conv2d_nntb.forward(I); 
%        I=BatchNorm(y,opts.l1,opts.l2,opts.parameters.batch_size);
%         ak=BatchNorm(sk,opts.l1,opts.l2,opts.parameters.batch_size);
%          y=sign(ak);
%          if opts.current_layer==12
%          opts.I12=I1;
...
%         y=BatchNorm(y,opts.l1,opts.l2,opts.parameters.batch_size);
        gradients = conv2d_nntb.gradients(I, y);
        dzdw=gradients{1}./opts.parameters.batch_size;              
        dzdb=reshape(gradients{2}./opts.parameters.batch_size,size(bias));
%         dzdb=zeros(size(bias));
        if(flip_kernel)
...
%     W=abs(opts.layer{opts.current_layer}.fk);
%     Alpha=mean(W);
%     B=sign(opts.layer{opts.current_layer}.fk);
%    opts.layer{opts.current_layer}.W=Alpha.*B;

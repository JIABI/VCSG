function [  net,res,opts ] = sgd2_SVRG_unb(  net,res,opts )
...
%         net.layers{1,layer}.weights{1, 7}=(dzdw-net.layers{1, layer}.weights{1, 3});
%         res(layer).dzdw=(1-opts.parameters.l1_lr_3)*net.layers{1,layer}.weights{1, 7}+(opts.parameters.l1_lr_3)*(net.layers{1, layer}.weights{1, 6});    

%          signal=ismember(opts.ep,opts.xrange);       
%            if signal==1
% % % % % % % % %              if  opts.parameters.l1_lr_3==1
% % % % % % % % %              Dzdw=normalize(dzdw,'center','mean');
% % % % % % % % %              res(layer).dzdw=Dzdw;               
% % % %              elseif  opts.parameters.l1_lr_3==0
%              Dzdw=normalize(dzdw,'center','mean');
%              res(layer).dzdw=Dzdw;  
% % % % % % % % %               else
%         net.layers{1,layer}.weights{1, 7}=dzdw;
%        variance=normalize(net.layers{1, layer}.weights{1,10},'center','median');
%         net.layers{1, layer}.weights{1,3}=((opts.parameters.l1_lr_3)*( net.layers{1, layer}.weights{1,10}-net.layers{1, layer}.weights{1, 6}));


if opts.parameters.l1_lr_3==0.5
     variance=((net.layers{1, layer}.weights{1,3}-net.layers{1, layer}.weights{1, 6}));
     net.layers{1, layer}.weights{1,5}=variance;
         res(layer).dzdw=0.5*(dzdw-variance); 
else
     variance=((opts.parameters.l1_lr_3)*(net.layers{1, layer}.weights{1,3}-net.layers{1, layer}.weights{1, 6}));
     net.layers{1, layer}.weights{1,5}=variance;
...
%   if opts.ep<=1
%              opts.Dzdw=((opts.parameters.l1_lr_3)*(dzdw-net.layers{1, layer}.weights{1, 6})).^(2); 
%   else
%             opts.dzdw=((net.layers{1, layer}.weights{1, 3}-net.layers{1, layer}.weights{1, 6})).^(2);
% %   end
%             opts.Dzdw1=sum(sum(sum(sum(opts.dzdw,2),1),3),4);
%             opts.Dzdw=opts.Dzdw1/opts.n_train;
%             opts.Dzdw= gather(opts.Dzdw);
%             opts.Dzdw=sqrt(opts.Dzdw);
% % % %              Dzdw1=normalize(Dzdw,'center','mean');
% % % %              res(layer).dzdw=Dzdw1; 
%         res(layer).dzdw=dzdw./abs(normalize(dzdw,4));      
% % % % % % % % %               end
%            else
%         net.layers{1,layer}.weights{1, 7}=(dzdw);
%         res(layer).dzdw=(1-opts.parameters.l1_lr_3)*dzdw-(opts.parameters.l1_lr_3)*(net.layers{1, layer}.weights{1, 3}-net.layers{1, layer}.weights{1, 6}); 
%            end    
...
%          end
         ...
         weight=net.layers{1}.weights{1};
         Weight=sqrt(sum(weight.^(2),3));
...
%          end
%          opts.fx(1,opts.grad1)=(fx(5,1,1,1));
%        net.layers{1,layer}.weights{1, 7}=(net.layers{1, layer}.weights{1,7}+res(layer).dzdw);
        end
    end
